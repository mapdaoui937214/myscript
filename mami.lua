...
local function setupCharacter(char)
    ...
    hum:GetPropertyChangedSignal("WalkSpeed"):Connect(function() if hum.WalkSpeed ~= values.ws then hum.WalkSpeed = values.ws end end)
    hum:GetPropertyChangedSignal("JumpPower"):Connect(function() if hum.JumpPower ~= values.jp then hum.JumpPower = values.jp end end)
...
local function createSlider(name, min, max, def, key)
    ...
    local function update()
        ...
        values[key] = val
        if key == "ws" and player.Character then player.Character.Humanoid.WalkSpeed = val end
    end
