...
KeySubmit.MouseButton1Click:Connect(function()
    ...
    if found then KeyGui:Destroy() ScreenGui.Enabled = true
    else
        KeyInput.Text = ""
        KeyInput.PlaceholderText = "Keyが違います"
        KeyInput.PlaceholderColor3 = Color3.new(1, 0, 0)
        task.delay(1.5, function() if KeyInput then KeyInput.PlaceholderText = "Keyを入力" KeyInput.PlaceholderColor3 = Color3.new(0.7, 0.7, 0.7) end end)
    end
end)

local Main = Instance.new("Frame", ScreenGui)
Main.Size, Main.Position, Main.BackgroundColor3 = UDim2.new(0, 220, 0, 400), UDim2.new(0.05, 0, 0.3, 0), Color3.fromRGB(20, 20, 20)
Main.Active, Main.Draggable = true, true
Instance.new("UICorner", Main)

local Scroll = Instance.new("ScrollingFrame", Main)
Scroll.Size, Scroll.Position, Scroll.BackgroundTransparency = UDim2.new(1, -10, 1, -50), UDim2.new(0, 5, 0, 45), 1
Scroll.CanvasSize, Scroll.ScrollBarThickness = UDim2.new(0, 0, 0, 1200), 5
...
local function applyStats(hum)
    ...
    hum.UseJumpPower, hum.JumpPower, hum.JumpHeight = true, values.jp, values.jp * 0.1
end
...
local function createSlider(name, min, max, def, key)
    ...
    local function update()
        ...
        local pos = math.clamp((inputPos - bar.AbsolutePosition.X) / bar.AbsoluteSize.X, 0, 1)
...
Title.Size, Title.Position, Title.Text = UDim2.new(1, -60, 0, 40), UDim2.new(0, 10, 0, 0), "まっぷhub Mobile"
Title.TextColor3, Title.Font, Title.TextSize, Title.BackgroundTransparency = Color3.new(1, 1, 1), Enum.Font.SourceSansBold, 18, 1
Title.TextXAlignment = Enum.TextXAlignment.Left

local CloseBtn = Instance.new("TextButton", Main)
CloseBtn.Size, CloseBtn.Position, CloseBtn.Text = UDim2.new(0, 30, 0, 30), UDim2.new(1, -35, 0, 5), "X"
CloseBtn.BackgroundColor3, CloseBtn.TextColor3 = Color3.fromRGB(180, 50, 50), Color3.new(1, 1, 1)
    ...
    if states.esp then
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= player and p.Character then
                ...
                hl.Name, hl.FillColor = "ESPHighlight", Color3.new(1, 0, 0)
                hl.Enabled = true
            end
        end
    else
        for _, p in pairs(Players:GetPlayers()) do
            if p.Character and p.Character:FindFirstChild("ESPHighlight") then p.Character.ESPHighlight.Enabled = false end
        end
    end
end)
