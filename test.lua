...
local ThemeDarkRed = Color3.fromRGB(110, 0, 0)
local ThemeGold = Color3.fromRGB(255, 215, 0)
local ThemeBlack = Color3.fromRGB(20, 20, 20)
...
RunService.Heartbeat:Connect(function()
    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if states.fly then
            ...
            hrp.FlyBG.CFrame = camera.CFrame
            hrp.FlyBV.Velocity = (hum.MoveDirection.Magnitude > 0) and (camera.CFrame:VectorToWorldSpace(camera.CFrame:VectorToObjectSpace(hum.MoveDirection)) * (values.fs * 25)) or Vector3.zero
        else
...
local function createSlider(name, min, max, def, key)
    ...
    bar.Size, bar.Position, bar.BackgroundColor3 = UDim2.new(1, 0, 0, 4), UDim2.new(0, 0, 0, 30), Color3.fromRGB(50, 0, 0)
    ...
    local function update()
        ...
        values[key] = val
        if player.Character then applyStats(player.Character.Humanoid) end
    end
    local drag = false
