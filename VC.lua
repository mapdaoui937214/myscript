...
MinBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        MainFrame:TweenSize(UDim2.new(0, 220, 0, 300), "Out", "Quad", 0.2, true)
        task.wait(0.2)
...
local function toggleSpy(target, btn)
    ...
    if activeWires[userId] then
        activeWires[userId]:Destroy()
        activeWires[userId] = nil
        ...
        if input and listener then
            ...
            pcall(function()
                ...
                if emitter then emitter.DistanceAttenuation = Enum.DistanceAttenuationCurve.Inverse end
            end)
            
            activeWires[userId] = wire
