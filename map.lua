...
ScreenGui.Name = "MapHub_V24_Final_Force"
...
createToggle("Force Fly (Fixed)", function(s) flying = s if player.Character then player.Character.Humanoid.PlatformStand = s end end)
...
RunService.Stepped:Connect(function(dt)
    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if flying then
            ...
            if UIS:IsKeyDown(Enum.KeyCode.W) or (hum.MoveDirection.Z < 0) then moveVec = moveVec + look end
            if UIS:IsKeyDown(Enum.KeyCode.S) or (hum.MoveDirection.Z > 0) then moveVec = moveVec - look end
            if UIS:IsKeyDown(Enum.KeyCode.A) or (hum.MoveDirection.X < 0) then moveVec = moveVec - right end
            if UIS:IsKeyDown(Enum.KeyCode.D) or (hum.MoveDirection.X > 0) then moveVec = moveVec + right end
            if UIS:IsKeyDown(Enum.KeyCode.Space) or (UIS.JumpTouchInput and not UIS:GetFocusedTextBox()) then moveVec = moveVec + Vector3.new(0, 1, 0) end
            if UIS:IsKeyDown(Enum.KeyCode.LeftControl) then moveVec = moveVec - Vector3.new(0, 1, 0) end
            
            if moveVec.Magnitude > 0 then
                hrp.CFrame = hrp.CFrame + (moveVec.Unit * flySpeed * dt)
            end
        end
        if noclip then
