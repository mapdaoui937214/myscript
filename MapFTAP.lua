...
SetSection:NewKeybind("Toggle UI", "Minimize (RightControl)", Enum.KeyCode.RightControl, function()
    Library:ToggleUI()
end)

SetSection:NewButton("Destroy Hub", "警告: 終了しますか？", function()
    Library:CreateLib("警告", "本当にHubを終了しますか？"):NewSection("確認"):NewButton("はい", "閉じる", function()
        for _, v in pairs(game:GetService("CoreGui"):GetChildren()) do
            if v.Name == "map FTAP hub" or v:FindFirstChild("Main") then
                v:Destroy()
            end
        end
    end)
    Library:CreateLib("警告", "本当にHubを終了しますか？"):NewSection("確認"):NewButton("いいえ", "戻る", function()
        print("Canceled")
    end)
end)
