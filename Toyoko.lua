        ...
        while running do
            ...
            if autoEnabled and targetName and humanoidRoot then
                ...
                if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if targetRoot.Position.Y > -60 then
                        fallen = false
...
local function refreshPlayerList()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr~=player then
            ...
            btn.TextSize=15
            btn.MouseButton1Click:Connect(function() targetName=plr.Name; title1.Text="🎯 Target: "..plr.DisplayName; fallen=false end)
...
local function updateButtons()
    if autoEnabled then
        autoButton1.Text="Auto: ON"; autoButton1.BackgroundColor3=Color3.fromRGB(0,200,100)
        toggleButton.Text="ON"; toggleButton.BackgroundColor3=Color3.fromRGB(0,200,100)
    else
        autoButton1.Text="Auto: OFF"; autoButton1.BackgroundColor3=Color3.fromRGB(60,60,60)
        toggleButton.Text="OFF"; toggleButton.BackgroundColor3=Color3.fromRGB(120,120,120)
    end
end
...
player.Chatted:Connect(function(msg)
    ...
    if msg:sub(1,7) == "target " then
        ...
        table.remove(args,1)
        if args[1] == "off" then
            multiTargets = {}; multiMode = false; autoEnabled = false; updateButtons()
            return
        end
        multiTargets = {}
        for _, arg in ipairs(args) do
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= player and (plr.Name:lower():find(arg) or plr.DisplayName:lower():find(arg)) then
                    table.insert(multiTargets, plr.Name); break
                end
            end
        end
        if #multiTargets > 0 then
            multiMode = true; currentIndex = 1; targetName = multiTargets[1]; autoEnabled = true; updateButtons()
        end
    end
...
local function handleCmd(plr, msg)
    if commanderName and plr.Name == commanderName then
        ...
        if text == "off" then autoEnabled = false; return end
